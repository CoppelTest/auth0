document.addEventListener('DOMContentLoaded', function () {
    console.log("-> 1. Script se inició.");
    ...
    try {
        ...
        // La lógica de los divs se deja aquí, pero es frágil.
        ...
        // --- SECCIÓN 2: Aplicación de estilos iniciales ---
        console.log("-> 3. Aplicando estilos y clases base.");

        mainElement.classList.remove('_widget', 'c7f43d5ad');
        if (mainElement.querySelector('h1')) mainElement.querySelector('h1').className = 'title-container';
        if (mainElement.querySelector('p')) mainElement.querySelector('p').className = 'subtitle-container';
        if (btnLogin) btnLogin.className = 'button-normal';
        if (inputCode) inputCode.className = 'email-input';
        if (mainElement.querySelector('div[data-action-text][data-alternate-action-text]')) {
            mainElement.querySelector('div[data-action-text][data-alternate-action-text]').className = '';
        }
        if (label) {
        ...
        if (document.querySelector('input[type="checkbox"]')) {
            ...
            const checkBoxContainer = document.querySelector('input[type="checkbox"]')?.closest('div');
        ...
        if (document.querySelector('button[name="action"][value="pick-authenticator"]')) {
            document.querySelector('button[name="action"][value="pick-authenticator"]').classList.add('link');
        }
...
function applyErrorStyles() {
    ...
    switch (errorType) {
        ...
        // --- SECCIÓN 4: Observación del DOM (la parte clave) ---
        // Se crea un observador para re-ejecutar la lógica si el DOM cambia
